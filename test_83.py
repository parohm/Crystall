...
def test_83():
    ...
    path = f'Cryst_measurements/{current_datetime.year}Y{current_datetime.month}M{current_datetime.day}D_N{cryst_number}_T{temperature}_Test83_{current_datetime.hour}h{current_datetime.minute}m{current_datetime.second}s'
    os.mkdir(path)

    for col in ['00010', '00110', '01001']:
        rowcel_and_colsel_positions(rowcel_config='00000', colcel_config=col)

        voltage_array = []
        blf_current_array = []
        for voltage in np.arange(0, 1801, 360):
            rohde_schwarz_2.channel_config(channel=3, voltage=0, current=0.001, output=True)

            rohde_schwarz_1.channel_config(channel=1, voltage=voltage/1000, current=vdd_current, output=True)

            rohde_schwarz_1.channel_config(channel=2, voltage=0, current=0.001, output=True)
        ...
        blf_current_df = pd.DataFrame(data=list(zip(voltage_array, blf_current_array)), columns=['Voltage, V', 'BLF_current, A'])
        blf_current_df.to_excel(path + f'/{current_datetime.year}Y{current_datetime.month}M{current_datetime.day}D_N{cryst_number}_T{temperature}_Test83_colsel{col}_BLFcurrent.xlsx')
